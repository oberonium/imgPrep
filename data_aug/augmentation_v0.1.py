...
def rotation(img, angle, scale):
    ...
    cv2.imwrite(img[:-4] + "-rotate.png", img_matrix_new)

    return


def rotate_label(labelXml, angle, scale, size):
    newXml_rot = labelXml[:-4] + "-rotate.xml"
    xmldata = etree.parse(labelXml)
    root = xmldata.getroot()
    width = int(root.find("size").find("width").text)
    height = int(root.find("size").find("height").text)
...
if __name__ == "__main__":
    ...
    rotation("/storage/dataprep_test/test_img/H_20.png", 20, 1)
    rotate_label("/storage/dataprep_test/Annotations/H_20.xml", 20, 1, "small")
